...
Quiz Engine Module
==================
In-memory quiz grading. No persistence.
Returns summary + preparation tips.
"""
...
from quiz.quiz_generator import Quiz, MCQQuestion


@dataclass
class QuizAttempt:
    """Quiz attempt record (in-memory only)."""
    attempt_id: str = ""
    quiz_id: str = ""
...
class QuizEngine:
    """In-memory quiz engine. Grades quiz, returns summary + preparation tips."""
    ...
    def grade_attempt(self, quiz_id: str, responses: List[Dict[str, Any]],
        ...
        """Grade quiz and return feedback."""
        quiz = self.get_quiz(quiz_id)
        if not quiz:
            raise ValueError(f"Quiz {quiz_id} not found")
        return self._grade(quiz, responses, user_id)

    def grade_attempt_in_memory(self, quiz: Quiz, responses: List[Dict[str, Any]],
                                 user_id: str = "default_user") -> QuizFeedback:
        """Grade a quiz object directly (no storage needed)."""
        return self._grade(quiz, responses, user_id)

    def _grade(self, quiz: Quiz, responses: List[Dict[str, Any]],
               user_id: str) -> QuizFeedback:
    ...
    def _generate_feedback(self, attempt: QuizAttempt, topic_mastery: Dict[str, float]) -> QuizFeedback:
        ...
        if score >= 90:
            ...
            summary = f"You scored {score}%. Don't worry — review the material and try again. You will improve!"
        else:
