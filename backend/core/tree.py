...
================================
Hierarchical tree structure for documents with chunks and node metadata.
"""
...
class DocumentTree:
    ...
    def get_chapter_nodes(self, chapter_node_id: str) -> List["TreeNode"]:
        """Get all nodes belonging to a specific chapter (includes chapter + descendants)."""
        chapter_nodes = []
    ...
    def get_chapters(self) -> List["TreeNode"]:
        ...
        if not root:
            return []
        return [self.nodes[cid] for cid in root.children_ids if cid in self.nodes]
