...
class MCTSNode:
    ...
    state: NodeState = NodeState.EXPANDED
...
class MonteCarloTreeSearch:
    ...
    def initialize(
        ...
        self.root_id = self._doc_to_mcts.get(root_node_id)
        return self.root_id
    ...
    def _get_mcts_node(self, doc_id: str) -> Optional[MCTSNode]:
        ...
        return self.nodes.get(mcts_id) if mcts_id else None
    ...
    def search(
        ...
        root_doc_id = self.nodes.get(self.root_id).document_node_id if self.root_id else None
    ...
    def _select_best_child_ucb(self, parent_doc_id: str) -> Optional[str]:
        """UCB = Q-value + C * sqrt(log(N_parent) / N_child)"""
        ...
        for child_doc_id in doc_node.children_ids:
            ...
            if child_mcts.visit_count == 0:
                q_value = child_mcts.prior_probability
                exploration = self.exploration_constant * math.sqrt(math.log(parent_visits))
            else:
                q_value = child_mcts.mean_value
                exploration = self.exploration_constant * math.sqrt(
                    math.log(parent_visits) / child_mcts.visit_count
                )

            score = q_value + exploration + random.random() * 0.001
            if score > best_score:
                best_score = score
    ...
    def _rollout(self, start_doc_id: str, query: str) -> float:
        ...
        while current_depth < max_rollout_depth:
            ...
            for child_doc_id in doc_node.children_ids:
                child_mcts = self._get_mcts_node(child_doc_id)
                if child_mcts:
                    candidates.append((child_doc_id, max(child_mcts.prior_probability, 0.01)))
            ...
            current_doc_id = candidates[np.random.choice(len(candidates), p=probs)][0]
            current_depth += 1

        final_mcts = self._get_mcts_node(current_doc_id)
        return max(final_mcts.prior_probability, 0.1) if final_mcts else 0.1
    ...
    def _extract_results(self, max_results: int) -> List[SearchResult]:
        ...
        for mcts_node in self.nodes.values():
            ...
            if mcts_node.visit_count == 0:
                score = mcts_node.prior_probability * 0.5
            else:
                score = (
                    self.value_weight * mcts_node.mean_value +
                    (1 - self.value_weight) * mcts_node.prior_probability
                )
